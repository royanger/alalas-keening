    ...
    <div>
      <div className="flex flex-row items-center mt-8">
        <Link href="/dashboard/inbox">
          <Button className="relative"> <FontAwesomeIcon icon={faChevronLeft} className="mr-3 top-[2px] relative" />Back </Button>
        </Link>
      ...
      <div className="border border-gray-300 dark:border-gray-700 mt-6">
        <div className="grid grid-cols-12 gap-2 p-8">
          ...
          <p className="col-span-11">{note.data[0].attributes.regarding}</p>
          <p className="col-span-1">To:</p>
          <p className="col-span-11" >{note.data[0].attributes.to}</p>
          <p className="col-span-1">From:</p>
          <p className="col-span-11">{note.data[0].attributes.from}</p>
        </div>
            ...
            note.data[0].attributes.attachments.data.map((attachment: any) => {
              return (
                <Link
                  ...
                  target="_blank"
                  className="h-16 border border-primary hover:bg-muted flex items-center justify-center mx-2 rounded p-4"
                >
